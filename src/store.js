...
export const store = reactive({
  screen: 'start',
  uid: null,
  user: { name: '', email: '', gender: 'Male', age: '', weight: '', height: '', goal: '' },
...
export function openPlanModal(id) {
  const src = id != null ? store.plans.find((p) => p.id === id) : { name: '', schedule: '', exercises: [] }
  store.planDraft = {
    id: id ?? null,
    name: src.name,
...
export function savePlanDraft() {
  ...
  if (d.id != null) {
    ...
    store.plans.push({ id: store.nextPlanId++, name, schedule: d.schedule.trim(), active: false, exercises: cleanExercises })
  }
  saveData()
  closeModal()
}
...
export function openExerciseModal(exId) {
  const plan = store.plans.find((p) => p.id === store.currentPlanId)
  const src = exId != null ? plan.exercises.find((e) => e.id === exId) : { name: '', sub: '' }
...
export function saveExerciseDraft() {
  ...
  if (d.id != null) {
    ...
    const nextId = plan.exercises.length ? Math.max(...plan.exercises.map((e) => e.id)) + 1 : 1
...
export function openMealModal(id) {
  const src = id != null ? store.meals.find((m) => m.id === id) : { name: '', schedule: '', items: [] }
  store.mealDraft = {
    id: id ?? null,
    name: src.name,
    schedule: src.schedule,
...
export function saveMealDraft() {
  ...
  const cleanItems = d.items
    .map((i) => ({ id: i.id, name: String(i.name).trim(), kcal: parseInt(i.kcal) || 0 }))
    .filter((i) => i.name)
  if (d.id != null) {
    ...
    store.meals.push({ id: store.nextMealId++, name, schedule: d.schedule.trim(), active: false, items: cleanItems })
  }
  saveData()
  closeModal()
}
...
export function openFoodItemModal(itemId) {
  const meal = store.meals.find((m) => m.id === store.currentMealId)
  const src = itemId != null ? meal.items.find((i) => i.id === itemId) : { name: '', kcal: '' }
...
export function saveFoodItemDraft() {
  ...
  if (d.id != null) {
    ...
    const nextId = meal.items.length ? Math.max(...meal.items.map((i) => i.id)) + 1 : 1
...
export function logout() {
  signOut(auth).catch(() => {})
  store.user = { name: '', email: '', gender: 'Male', age: '', weight: '', height: '', goal: '' }
  store.screen = 'auth'
}
